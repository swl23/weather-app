...
async function runApp() {
    resetDisplay();
    hideElement(document.getElementById("no-weather"));
...
async function lookUpWeather(city) {
    const url =
        apiPrefix + formatAddress(city) + "?key=AWEPLTSDVJWX27V9PGFLFUG8A";
    ...
    rawData.days
        .slice(0, 7)
        .forEach((day) => weekForecast.push(forecastData(day)));
...
function displayCurrent(current) {
    ...
    feelsLike.textContent =
        "Feels like " + displayTempInChosenScale(current["Feels Like"]);
...
function formatDate(dateObject) {
    const month = `${dateObject[5]}${dateObject[6]}`;
    const date = `${dateObject[8]}${dateObject[9]}`;
    const year = `${dateObject[0]}${dateObject[1]}${dateObject[2]}${dateObject[3]}`;
...
function displayWeek(week) {
    ...
    prevBtn.setAttribute("class", "slide-arrow");
    prevBtn.textContent = "<";

    const nextBtn = document.createElement("button");
    nextBtn.setAttribute("id", "slide-arrow-next");
    nextBtn.setAttribute("class", "slide-arrow");
    nextBtn.textContent = ">";
    ...
    week.forEach((day) => {
        ...
        lowTemp.textContent = displayTempInChosenScale(
        ...
        highTemp.textContent = displayTempInChosenScale(
...
function pageLoad() {
    resetDisplay();
    showElement(document.getElementById("no-weather"));
}
...
function resetDisplay() {
    ...
    noWeather.setAttribute("id", "no-weather");
    noWeather.textContent = "Search a location to see the weather.";
    hideElement(noWeather);
    current.appendChild(noWeather);
